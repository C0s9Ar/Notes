...
class Note(db.Model):
    ...
    date = db.Column(db.String(100), nullable=False)  # Date as a string
...
def get_notes():
    ...
    return jsonify([{'title': note.title, 'text': note.text, 'author': note.author, 'date': note.date} for note in notes])
